...
```{r}
d_fulltext = read_rds("Lecture_10/text_mining/data/data_ce_speech_article.rds")
...
```{r}
d_tokenized_s_nrc = d_tokenized_s |> group_by(uid) |> mutate(doc_length = n()) |> ungroup() |> inner_join(dict_nrc, by = "word", multiple = "all")

d_tokenized_s_nrc_agg = d_tokenized_s_nrc |> group_by(uid, date_of_speech, sentiment) |> summarise(n = n() / mean(doc_length)) |> pivot_wider(names_from = "sentiment", values_from = "n", names_prefix = "sentiment_score_")
```
